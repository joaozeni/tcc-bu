...
\textoResumo {Q problema SAT é um problema clássico e bem conhecido, sendo o primeiro provado ser NP-completo
...
Neste capítulo são descritos os fundamentos necessários para a compreensão do trabalho 
...
Por ultimo é apresentado o algoritmo base utilizado neste trabalho.

\section{O Problema SAT}
\label{sec:sat}

O problema SAT consiste em saber se há alguma atribuição de valores verdade que satisfaz uma 
fórmula lógica proposicional descrita em CNF. Iniciamos este capitulo apresentando um pouco 
...
canônicas, a \textit{forma normal conjuntiva} (CNF, sigla em inglês) e a 
...
Será utilizado como base o algoritmo \textit{SAT solver} QuantumSAT\allowbreak 
 \cite{bittencourt2003syntactic}. Este é um algoritmo \textit{SAT solver} completo, 
 cuja técnica para a solução baseia-se em converter uma fórmula proposicional 
...
forma paralela. Primeiramente é apresenta a técnica portfólio, que consiste em utilizar 
...
ao estado, e o gap, que é o conjunto de clausulas que não contém um literal no $\Omega$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figuras/results_glucose.png}
    \caption{Comparação com o Glucose(multi-core)}
    \label{fig:port}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figuras/results_pqsat.png}
    \caption{Comparação com o ParaQuantumSAT(multi-core)}
    \label{fig:port}
\end{figure}
...
Também é possível realizar alguns estudos de melhoria da implementação. Podem-se estudar as 
